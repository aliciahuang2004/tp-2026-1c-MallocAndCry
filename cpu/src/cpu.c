...
void ciclo_de_instruccion(t_cpu *cpu, t_contexto *contexto)
{
    ...
    while (ejecutando)
    {
        ...
        // limpieza
        free(cadena_leida);
        if (instruccion_actual.nombre_operacion)
            free(instruccion_actual.nombre_operacion);
        if (instruccion_actual.argumento_operando_destino)
            free(instruccion_actual.argumento_operando_destino);
        if (instruccion_actual.argumento_operando_origen)
            free(instruccion_actual.argumento_operando_origen);
    }
}
...
t_instruccion_decodificada decodificar_instruccion(t_cpu *cpu, char *cadena_instruccion_texto)
{
    ...
    if (tokens_de_la_linea[0] != NULL)
    {
        ...
        if (strcmp(tokens_de_la_linea[0], "NOOP") == 0)
            instruccion_formateada.identificador_operacion = INST_NOOP;
        else if (strcmp(tokens_de_la_linea[0], "SET") == 0)
            instruccion_formateada.identificador_operacion = INST_SET;
        else if (strcmp(tokens_de_la_linea[0], "MOV_IN") == 0)
            instruccion_formateada.identificador_operacion = INST_MOV_IN;
        else if (strcmp(tokens_de_la_linea[0], "MOV_OUT") == 0)
            instruccion_formateada.identificador_operacion = INST_MOV_OUT;
        else if (strcmp(tokens_de_la_linea[0], "SUM") == 0)
            instruccion_formateada.identificador_operacion = INST_SUM;
        else if (strcmp(tokens_de_la_linea[0], "SUB") == 0)
            instruccion_formateada.identificador_operacion = INST_SUB;
        else if (strcmp(tokens_de_la_linea[0], "JNZ") == 0)
            instruccion_formateada.identificador_operacion = INST_JNZ;
        else if (strcmp(tokens_de_la_linea[0], "COPY_MEM") == 0)
            instruccion_formateada.identificador_operacion = INST_COPY_MEM;
        else if (strcmp(tokens_de_la_linea[0], "MUTEX_CREATE") == 0)
            instruccion_formateada.identificador_operacion = INST_MUTEX_CREATE;
        else if (strcmp(tokens_de_la_linea[0], "MUTEX_LOCK") == 0)
            instruccion_formateada.identificador_operacion = INST_MUTEX_LOCK;
        else if (strcmp(tokens_de_la_linea[0], "MUTEX_UNLOCK") == 0)
            instruccion_formateada.identificador_operacion = INST_MUTEX_UNLOCK;
        else if (strcmp(tokens_de_la_linea[0], "MEM_ALLOC") == 0)
            instruccion_formateada.identificador_operacion = INST_MEM_ALLOC;
        else if (strcmp(tokens_de_la_linea[0], "MEM_FREE") == 0)
            instruccion_formateada.identificador_operacion = INST_MEM_FREE;
        else if (strcmp(tokens_de_la_linea[0], "SLEEP") == 0)
            instruccion_formateada.identificador_operacion = INST_SLEEP;
        else if (strcmp(tokens_de_la_linea[0], "STDOUT") == 0)
            instruccion_formateada.identificador_operacion = INST_STDOUT;
        else if (strcmp(tokens_de_la_linea[0], "STDIN") == 0)
            instruccion_formateada.identificador_operacion = INST_STDIN;
        else if (strcmp(tokens_de_la_linea[0], "INIT_PROC") == 0)
            instruccion_formateada.identificador_operacion = INST_INIT_PROC;
        else if (strcmp(tokens_de_la_linea[0], "EXIT") == 0)
            instruccion_formateada.identificador_operacion = INST_EXIT;
...
void desconectar_memory_stick(t_cpu* cpu, int ms_id) {
    ...
    for (int i = 0; i < list_size(cpu->sockets_memory_sticks); i++) {
        ...
        if (ms->id == ms_id) {
            log_warning(cpu->logger, "## Detectada desconexión de Memory Stick (ID: %d)", ms_id);
